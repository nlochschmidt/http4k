...
abstract class ServerStopContract(
    ...
    private val defaultGracefulStopMode = Graceful(Duration.ofSeconds(3))
    private val defaultDelayedStopMode = Delayed(Duration.ofSeconds(3))
    private val timeoutTolerance = Duration.ofMillis(100)

    private val supportedStopModes: Set<StopMode>


    init {
        supportedStopModes = ConfigureServerStopContract(enabledModes = mutableSetOf(Immediate))
            .also(enableStopModes)
            .enabledModes
    }

    inner class ConfigureServerStopContract(val enabledModes: MutableSet<StopMode>) {
    ...
    private val routes =
        listOf(
            ...
            "/slow-echo" bind POST to slowEchoHandler(Duration.ofMillis(500))
        )
    ...
    private fun startServerOrSkip(stopMode: StopMode): Http4kServer {
        ...
        return prepareServer(stopMode).start()
    }
    ...
    fun `immediate stop mode is cancelling inflight requests`() {
        assertThrows<AssertionError> {
            startServerOrSkip(Immediate).testInflightRequestsCompleteDuringServerStop()
        }
    }
